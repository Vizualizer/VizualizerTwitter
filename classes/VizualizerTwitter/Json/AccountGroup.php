...
class VizualizerTwitter_Json_AccountGroup extends VizualizerTwitter_Json_Account
{
    public function execute()
    {
        ...
        if($post["account_id"] > 0 && $post["index"] > 0){
            $accountGroup->updateAccountGroup($post["account_id"], $post["index"], $post["value"]);
        }elseif($post["account_id"] > 0 && $post["old_value"] != $post["value"]){
            $accountGroup->changeAccountGroup($post["account_id"], $post["old_value"], $post["value"]);
        }
