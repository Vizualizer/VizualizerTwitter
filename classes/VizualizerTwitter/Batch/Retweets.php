...
class VizualizerTwitter_Batch_Retweets extends Vizualizer_Plugin_Batch
{
    ...
    protected function postRetweets($params, $data)
    {
        ...
        // 本体の処理を実行
        $retweets = $model->findAllBy(array("retweet_tweet_id" => "", "le:scheduled_retweet_time" => Vizualizer::now()->date("Y-m-d H:i:s"), "retweet_time" => "0000-00-00 00:00:00"), "scheduled_retweet_time", false);
        ...
        // キャンセルの本体の処理を実行
        $retweets = $model->findAllBy(array("ne:retweet_tweet_id" => "", "le:scheduled_cancel_retweet_time" => Vizualizer::now()->date("Y-m-d H:i:s"), "ne:retweet_tweet_id" => "0", "cancel_retweet_time" => "0000-00-00 00:00:00"), "scheduled_retweet_time", false);
        foreach ($retweets as $retweet) {
            $account = $retweet->account();
